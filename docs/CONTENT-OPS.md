...
- `news.ts` は84件。b66-01 は id `2026-09-08-stream-thanks-morning-slot-story`、sameDayOrder: 10、
  activityIds: live-stream。9/8 の先頭に置く。
- HOME Latest / `/news/` と Gallery が公開MP4 1本・poster 1枚・manifest object 1件を共有する。
  `galleryVideos.ts` は独立動画33本＋Mixch outbound player 4本で、b66-01 を先頭へ置く。
...
### 2026-09-08 Instagram Story 「みりぽち待ってます」（batch b79）

- オーナーがチャットで直接提供した本人Instagram Story動画1本。SNSから再取得していない。
- `news.ts` は85件。b79-01 は id `2026-09-08-miripochi-waiting-vote-story`、sameDayOrder: 20、
  activityIds: miss-circle。同日の配信お礼Story（b66-01、sameDayOrder: 10）より前に置く。
- HOME Latest / `/news/` と Gallery が公開MP4 1本・poster 1枚・manifest object 1件を共有する。
  `galleryVideos.ts` は独立動画34本＋Mixch outbound player 4本で、b79-01 を先頭へ置く。
  MISS CIRCLE Activity の関連NEWS・関連メディアにも出る。`/stories/`、highlights、events、
  streamSchedule、`media.ts`、contest.ts には追加しない。
- source date / NEWS日付 `2026-09-08` は、元動画のcontainer creation_time（2026-09-08 14:20 JST。
  ファイルメタデータであり投稿時刻の確定値ではない）と、オーナーがその直後（約2分後）に
  提供したことによる。NEWS本文は「9月8日」とし、時刻・時間帯は書かない。
- 画面の文言はリンクスタンプ「みりぽち待ってます🩵🩵🩵」だけ。三次審査WEB投票期間
  （9/3〜9/13）中の呼びかけで、b65-02 の「5日目ポチッはこちらから」と同じ「ポチッ」系の
  表現のため「投票を呼びかけ」と要約した。「みりぽち」の意味や投票日数は本文で補わない。
  リンクスタンプの遷移先は画面から確認できないため、CTAは確認済みの `missCircleWebVoteLink`
  のみ。SupportEvent `miss-circle-2026-3rd-web-vote` の期間終了後は自動で消える。
- ヘッドホンとマイクが写るが、場所・番組・配信との関係は確認していないため、RADIO /
  LIVE STREAM Activity には関連付けず、本文でも場所を断定しない。
- 恒久permalinkがないため、出典は非リンクの `Instagram Story` label。本人Instagram
  プロフィールは関連CTAであり、Storyの出典URLとして扱わない。SHOWROOM / Paton CTAは付けない。
- 公開派生は video-only。元素材の HE-AAC 音声は再配信権を確認できないため外し、映像は
  H.264 High / 720×1280 / 1fps / 20フレーム を `-c:v copy` で remux し、再エンコード・crop・
  scale・短縮・AI加工はしない。
- mainでbatch b67〜b78 が配信スクショ等で先に使用されているため、この Story 動画は b79 として採番する。
