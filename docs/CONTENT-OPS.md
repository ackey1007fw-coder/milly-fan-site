...
## 2026-09-09 本人X 朝枠のお礼・夜枠案内（PR #231）

- 一次出典は本人X `https://x.com/Mily_chan36/status/2097492757690646747`。NEWS日付は元投稿日 `2026-09-09` を維持し、台帳補完日（2026-09-10）へ変更しない。
- `news.ts` のid `2026-09-09-morning-thanks-night-stream` は1件。2026-09-10にcurrent mainと照合したNEWS総数は87件。日付降順では9/8のStoryより前に表示する。
- 朝枠へのお礼、次枠21:30〜23:00、MISS CIRCLE 3次審査中の応援呼びかけを要約。`activityIds` は `live-stream`, `miss-circle`。一次出典のXと、関連CTA `https://www.showroom-live.com/r/circle2026_0734` を区別する。揮発する残pt・`is_live`・`next_live` は本文へ固定しない。
- 同じ確認済み告知に合わせ、`streamSchedule.ts` の9/9 21:30枠だけ `endTime: "21:50"` から `"23:00"` へ訂正。9/10時点では終了済みで現在・今後の枠を増やさない。自動取得成功時の公式枠優先、終了時刻以降の非表示、他の日・時間帯は変更しない。
- 承認済みの関連動画 `showroomAvatarRightsStoryVideo`（b92）は `sourceDate: null`。merged PR #229と `docs/MEDIA.md` は投稿日未確認のためGalleryのみと明記している。同じ朝・夜枠の文言だけからStory投稿日を推測せず、今回のNEWSへは配線しない。日付が一次確認できた段階で既存MP4・posterを再利用できるが、恒久的なメディア禁止ではない。
- `events.ts` / `media.ts` / `socials.ts` / `galleryVideos.ts` / `/stories/` / highlights / profile / contestへは追加しない。新しい写真・動画・URL、SNS CDN参照、スクレイピング、素材の複製・再生成はない。NEWS本文と既存Gallery素材・掲載面は維持する。
